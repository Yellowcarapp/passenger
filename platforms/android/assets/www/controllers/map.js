...
angular.module('MyApp',[[ 'lib/ion-places/ion-place-autocomplete.css',
    ...
    $scope.check_LimitCanceltrip=function(tripAcceptDate){
         ...
         if(tripAcceptDate && $scope.CurrentTripData && ($scope.CurrentTripData.tripStatus==3 ||$scope.CurrentTripData.tripStatus==4)){
            ...
            if(timepermin>$rootScope.Config.LimitCanceltrip) flag= true;
         }
    ...
    $scope.MapSettings=function()
    {
        ...
        .success(function(res){
            ...
            {
                if($scope.CurrentTripData.tripStatus==1){
                   $scope.ShowSearchLoading=true;
                    ...
                    else if($scope.CurrentTripData.tripStatus==6){CurrentType='dropoff'}
    ...
    $scope.openpayment= function(){
        console.log($rootScope.Payment_Page)
              ...
              case 'accepted':
                ...
                case 'dropoff':
                    ...
                    if($rootScope.marker_postion){
                      $scope.add_markerlocation($rootScope.marker_postion.latitude,$rootScope.marker_postion.longitude);
   ...
   socket.on('update',function (data){
    ...
    $scope.add_markerlocation=function(lat,lng){
        ...
        }else if($scope.passenger_marker){
          $scope.passenger_marker.setPosition(new plugin.google.maps.LatLng(lat, lng));
    ...
    $scope.$on("$ionicView.afterEnter", function(event, data){
        ...
        map.on(plugin.google.maps.event.MAP_READY, function(){
             $scope.add_markerlocation(0.0,0.0);
            map.getMyLocation(MyLocationoption,function(location) {
